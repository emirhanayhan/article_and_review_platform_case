...
async def test_user_create(client):
    with client:
        ...
        response = client.post("/api/v1/tokens", json=user_payload)
        ...
        assert response.json().get("password", "not_exists") == "not_exists"
        assert response.json().get("email") == "johniewalker@gmail.com"
